...
router.get('/tracks', requireAuth, requireCreator, (req, res) => {
  const all = store.getAllTracksIncludingDeleted();
  const myTracks = all.filter(t => t.uploadedByUserId === req.user.id);
  res.json({ tracks: myTracks });
});

// 내 음원 정보 수정
router.patch('/tracks/:trackId', requireAuth, requireCreator, (req, res) => {
  const track = store.getTrackById(req.params.trackId);
  ...
  const { title, genre, description, coverUrl, album, trackNumber, releaseYear } = req.body;
  const updates = {};
  if (title !== undefined) {
  ...
  if (trackNumber !== undefined) updates.trackNumber = String(trackNumber).trim();
  if (releaseYear !== undefined) updates.releaseYear = String(releaseYear).trim();

  const updated = store.updateTrack(req.params.trackId, updates);
  res.json({ message: '음원 정보가 수정되었습니다.', track: updated });
});

// 삭제 요청 생성
router.post('/tracks/:trackId/delete-request', requireAuth, requireCreator, (req, res) => {
  const track = store.getTrackById(req.params.trackId);
  if (!track) return res.status(404).json({ error: '트랙을 찾을 수 없습니다.' });
  if (track.uploadedByUserId !== req.user.id) {
  ...
  if (deleteReqStore.findPendingByTrackId(track.id)) {
    return res.status(409).json({ error: '이미 대기 중인 삭제 요청이 있습니다.' });
  }

  const { reason } = req.body;
  const request = deleteReqStore.create({
    trackId: track.id,
...
// 내 삭제 요청 목록 조회
router.get('/track-delete-requests', requireAuth, requireCreator, (req, res) => {
  const requests = deleteReqStore.getByCreatorUserId(req.user.id);
  res.json({ requests });
});
